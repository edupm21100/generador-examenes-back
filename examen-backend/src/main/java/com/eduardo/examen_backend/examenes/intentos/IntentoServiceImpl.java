...
public class IntentoServiceImpl implements IntentoService {
        ...
        for (RespuestaAlumnoDTO respuestaDTO : dto.getRespuestas()) {
            ...
            if (!preguntasYaRespondidas.add(respuestaDTO.getIdPregunta())) {
                throw new BadRequestException(
                        "Intento de fraude detectado: Se ha enviado más de una respuesta para la pregunta ID "
            ...
            Pregunta preguntaReal = preguntasActivas.stream()
                    ...
                    .findFirst()
                    .orElseThrow(() -> new BadRequestException("La pregunta ID " + respuestaDTO.getIdPregunta()
                            + " no pertenece a este examen o ha sido desactivada."));
            ...
            if (respuestaDTO.getIdOpcionSeleccionada() != null) {
                opcionElegida = preguntaReal.getOpciones().stream()
                        ...
                        .findFirst()
                        .orElseThrow(() -> new BadRequestException("La opción ID "
                                + respuestaDTO.getIdOpcionSeleccionada() + " no es válida para esta pregunta."));
    ...
    public IntentoDTO obtenerDetalleIntento(Integer idIntento, String correoLogueado) {
        ...
        if (!esDuenho && !esAdmin) {
            log.warn(
                    "[Alerta de Seguridad] El usuario {} intentó acceder al examen (Intento ID: {}) perteneciente a otro alumno.",
    ...
    @Override
    @Transactional(readOnly = true)
    public byte[] generarReporteIntentoPdf(Integer idIntento, String correoLogueado) {
        Intento intento = intentoRepository.findById(idIntento)
            .orElseThrow(() -> new NotFoundException("El intento no existe."));
        ...
        if (!esDuenho && !esAdmin) {
            log.warn("[Seguridad] {} intentó descargar PDF de otro alumno (Intento ID: {})", correoLogueado, idIntento);
            throw new BadRequestException("No tienes permiso para descargar este examen.");
        }

        Map<String, Object> variables = new HashMap<>();
        variables.put("intento", intento); 
        variables.put("fecha", LocalDate.now().toString());

        return pdfService.generarPdfDesdeHtml("reporte_intento", variables);
    }
        ...
        List<RespuestaAlumnoDTO> respuestasDTO = intento.getRespuestas().stream().map(r -> {
            ...
            return RespuestaAlumnoDTO.builder()
                    ...
                    .idOpcionSeleccionada(
                            r.getOpcionSeleccionada() != null ? r.getOpcionSeleccionada().getIdOpcion() : null)
                    .esCorrecta(correcta)
                    .build();
        }).toList();
